...
function uploadCloudCode(appID, secret, version, codeDir) {
  return new Promise((resolve, reject) => {
    console.log('Uploading cloud code...');
    const prefix = 'cloud';
    const walker = walk.walk(prefix);

    const zipFile = new yazl.ZipFile();

    zipFile.outputStream.pipe(request.post(`${aux}app/cloudcode/${version}.zip`, {
      headers: parseHeaders(appID, secret),
      auth: {
        user: appID,
        password: secret,
    ...
    walker.on("file", (root, fileStats, next) => {
      let relativeDirectory = root.replace(new RegExp(`^${prefix}/?`),"")
      zipFile.addFile(`${root}/${fileStats.name}`, `${relativeDirectory}${fileStats.name}`);
      next();
    });
...
function createVersionExecute(appID, secret, version, local, remote, codeDir) {
  return Promise.join(
    uploadMissing(appID, secret, local, remote),
    uploadCloudCode(appID, secret, version, codeDir),
    uploadMapping(appID, secret, version, local),
    () => setVersion(appID, secret, version))
  .then(() => console.log('All done!'));
}

function createVersion(appID, secret, version, codeDir = "cloud") {
  return Promise.join(
    hashWalk('public'),
    list(appID, secret),
    (local, remote) => createVersionExecute(appID, secret, version, local, remote, codeDir));
}
